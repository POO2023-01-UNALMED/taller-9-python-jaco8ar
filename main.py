...
pantalla.grid(row=0, column=0, columnspan=30, padx=1, pady=1)
...
print(type(pantalla.get()))

def AgregarTexto(evento):
    boton = evento.widget
    if boton["text"] == "=":
        rpta = eval(pantalla.get())
        pantalla.insert(0, "                                                  ")
        pantalla.insert(0, rpta)
    else:
        pantalla.insert(len(pantalla.get()), boton["text"])

for widget in root.winfo_children():
    if isinstance(widget, Button):
        widget.bind("<Button-1>",AgregarTexto)
